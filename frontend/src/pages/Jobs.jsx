    ...
    <div className="min-h-screen bg-[#010102]">
      ...
      <div className="px-8 py-8">
          ...
          >
            <div className="mb-3">
              ...
              <input
                ...
                max={10}
                value={nSamples}
                onChange={e => setNSamples(parseInt(e.target.value, 10) || 1)}
                className="bg-[#141516] border border-[#23252a] rounded-[8px] px-3 py-2 text-[#f7f8f8] text-sm focus:outline-none focus:border-[#5e6ad2] w-24"
              />
            </div>
            <div className="flex gap-2">
              <button
                type="submit"
                disabled={creating}
                className="px-3 py-1.5 bg-[#5e6ad2] hover:bg-[#828fff] text-white text-xs rounded-[8px] transition-colors disabled:opacity-50"
              >
              ...
              <button
                ...
                className="px-3 py-1.5 text-[#f7f8f8] text-xs rounded-[8px] hover:bg-[#141516] transition-colors"
              >
                Cancel
              </button>
            </div>
        ...
        {jobs && jobs.length > 0 && !isMock && (() => {
          ...
          const totalAnswers = jobs.reduce((s, j) => s + (j.answer_count || 0), 0)
          const totalClaims = jobs.reduce((s, j) => s + (j.claim_count || 0), 0)
          ...
          return (
            <div className="grid grid-cols-4 gap-3 mb-6">
              {[
                ...
                <div key={s.label} className="bg-[#0f1011] border border-[#23252a] rounded-[8px] p-4">
        ...
        {jobs && jobs.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center py-32 gap-4">
            <div className="w-12 h-12 rounded-[12px] bg-[#0f1011] border border-[#23252a] flex items-center justify-center">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#5e6ad2" strokeWidth="1.5">
                <path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586 a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19 a2 2 0 01-2 2z" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
            <p className="text-[#f7f8f8] text-sm font-medium">No analyses yet</p>
            <p className="text-[#8a8f98] text-xs text-center max-w-xs">
              Upload research papers to extract claims, score evidence-grounded answers, and chat over the results.
            </p>
            <button
              onClick={() => navigate('/jobs/new')}
              className="mt-2 bg-[#5e6ad2] hover:bg-[#828fff] text-white text-sm font-medium px-4 py-2 rounded-[8px] transition-colors"
            >
        ...
        {jobs && jobs.length > 0 && (
          <table className="w-full">
            ...
            <tbody>
              {jobs.map(job => (
                <tr key={job.id} className="border-b border-[#23252a] hover:bg-[#0f1011] transition-colors">
                  <td className="px-4 py-3 text-[#62666d] font-mono text-xs">{job.id}</td>
                  <td className="px-4 py-3">
                    <StatusBadge status={job.status} size="sm" />
                  </td>
                  <td className="px-4 py-3 text-[#d0d6e0] font-mono text-xs">{job.paper_count ?? 0}</td>
                  <td className="px-4 py-3 text-[#d0d6e0] font-mono text-xs">{job.claim_count ?? 0}</td>
                  <td className="px-4 py-3 text-[#d0d6e0] font-mono text-xs">{job.answer_count ?? 0}</td>
