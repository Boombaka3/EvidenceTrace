...
const STATUS_MSG = {
  ...
  RUNNING: { text: 'Extracting claims and answering evidence questions...', color: 'text-[#d0d6e0]' },
...
export default function JobStatus() {
  ...
  useEffect(() => {
    ...
    function poll() {
      getJob(id)
        ...
        .catch(() => {
          if (!cancelled) {
            setJob(MOCK_JOBS.find(j => String(j.id) === String(id)) || null)
            setIsMock(true)
            setPolling(false)
          }
        })
  ...
  useEffect(() => {
    if (job?.status === 'DONE') {
      setRedirecting(true)
      const t = setTimeout(() => navigate(`/jobs/${id}/chat`), 2000)
  ...
  if (!job && !isMock) {
    return (
      <div className="min-h-screen bg-[#010102] px-8 py-8 max-w-2xl mx-auto">
        <LoadingState rows={4} />
      </div>
    )
  }
  ...
  const paperCount = job?.paper_count ?? 0
  const claimCount = job?.claim_count ?? 0
  const answerCount = job?.answer_count ?? 0
  ...
  const stages = [
    ...
    { label: 'Done', done: job?.status === 'DONE' },
  ]

  const currentStageIdx = isRunning ? stages.findIndex(s => !s.done) : -1
    ...
    <div className="min-h-screen bg-[#010102]">
      <div className="px-8 py-8 max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          {isMock && <span className="text-[#F59E0B] text-xs font-mono">DEMO</span>}
        </div>
        ...
        <div className="bg-[#0f1011] border border-[#23252a] rounded-[12px] p-6 space-y-5">
          <div className="flex items-center gap-3">
            ...
            {polling && <span className="w-1.5 h-1.5 bg-[#5e6ad2] rounded-full animate-pulse" />}
          </div>
        ...
        <div className="mt-4 bg-[#0f1011] border border-[#23252a] rounded-[12px] p-5">
          ...
          <div className="space-y-3">
            {stages.map((stage, i) => {
              const state = stage.done ? 'done' : i === currentStageIdx ? 'current' : 'pending'
              return (
        ...
        <div className="mt-4 flex gap-3 flex-wrap">
          {job?.status === 'DONE' && (
            <>
              <Link
                to={`/jobs/${id}/answers`}
                className="px-4 py-2 bg-[#5e6ad2] hover:bg-[#828fff] text-white text-sm rounded-[8px] transition-colors"
              >
                QA Answers {'->'}
              </Link>
              <Link
                to={`/jobs/${id}/chat`}
                className="px-4 py-2 bg-[#0f1011] border border-[#23252a] hover:border-[#5e6ad2] text-[#d0d6e0] hover:text-[#f7f8f8] text-sm rounded-[8px] transition-colors font-mono"
              >
                Chat with Evidence {'->'}
              </Link>
            </>
          ...
          {job?.status === 'FAILED' && (
            <Link
              to={`/jobs/${id}`}
              className="px-4 py-2 bg-[#0f1011] border border-[#23252a] hover:border-[#34343a] text-[#d0d6e0] text-sm rounded-[8px] transition-colors"
            >
              Back to Papers
            </Link>
          )}
